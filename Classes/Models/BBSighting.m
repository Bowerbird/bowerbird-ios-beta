...
-(void)setObservedOnDate:(NSDate *)observedOnDate { _observedOnDate = observedOnDate; }
-(NSDate*)observedOnDate { return _observedOnDate; }
...
-(id)objectInCommentsAtIndex:(NSUInteger)index { return [_comments objectAtIndex:index]; }
-(void)setProjects:(NSArray *)projects { _projects = projects; }
-(NSArray*)projects {
    if(!_projects) _projects = [[NSArray alloc]init];
    return _projects;
}
-(NSUInteger)countOfProjects { return [_projects count]; }
-(id)objectInProjectsAtIndex:(NSUInteger)index { return [_projects objectAtIndex:index]; }
-(void)setProjectIds:(NSSet *)projectIds { _projectIds = projectIds; }
-(NSSet*)projectIds {
    if(!_projectIds)_projectIds = [[NSSet alloc]init];
    return _projectIds;
}
-(NSUInteger)countOfProjectIds { return [self.projectIds count]; }
-(NSEnumerator*)enumeratorOfProjectIds { return [self.projectIds objectEnumerator]; }
-(NSString*)memberOfProjectIds:(NSString *)object { return [self.projectIds member:object]; }
