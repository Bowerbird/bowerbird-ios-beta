...
+ (PhotoBox *)mediaForMap:(NSString *)url size:(CGSize)size {
    ...
    __block id bbox = box;
    box.asyncLayoutOnce = ^{
        [bbox loadPhotoFromLocation:url fromBowerBird:NO];
    };
...
+ (PhotoBox *)mediaFor:(NSString*)mediaUrl size:(CGSize)size {
    ...
    __block id bbox = box;
    box.asyncLayoutOnce = ^{
        [bbox loadPhotoFromLocation:mediaUrl fromBowerBird:YES];
    };
...
- (void)loadPhotoFromLocation:(NSString*)photoUrl fromBowerBird:(BOOL)bowerbird {
    ...
    // photo url
    id photosDir = [BBConstants RootUriString];
    id fullPath;
    if(bowerbird){
        fullPath = [NSString stringWithFormat:@"%@%@", photosDir, photoUrl];
    }else{
        fullPath = photoUrl;
    }
